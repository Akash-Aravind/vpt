...
def search_results(request):
    ...
    final_queryset = objs.union(secondobjs)[:30]
    dummy_queryset=DICT.objects.filter(values__icontains="nothing")

    if(q.strip()==""):
        return render(request,"searchresult.html",{"final_queryset":dummy_queryset,"q":q})
